...
# -*- coding: utf-8 -*-
"""
Auto commit script for any Frappe app.
...
def get_app_name():
    """Auto-detect app name from current directory or pyproject.toml."""
    base_dir = os.path.dirname(__file__)

    # Try to get app name from pyproject.toml
    pyproject_file = os.path.join(base_dir, "pyproject.toml")
    if os.path.exists(pyproject_file):
        with open(pyproject_file, 'r', encoding='utf-8') as f:
            content = f.read()
            # Look for name = "app_name" pattern
            import re
...
def update_version():
    """Update __version__ in {app}/__init__.py and README.md."""
    base_dir = os.path.dirname(__file__)
    app_name = get_app_name()

    init_file = os.path.join(base_dir, app_name, "__init__.py")
